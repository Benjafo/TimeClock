...
const { checkAdminPermission, formatDuration } = require('../utils/permissions');
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (summaryData.byPerson && Object.keys(summaryData.byPerson).length > 0) {
            let personText = '';
            const sortedPeople = Object.entries(summaryData.byPerson)
        ...
        // Summary by project
        if (summaryData.byProject && Object.keys(summaryData.byProject).length > 0) {
            let projectText = '';
            const sortedProjects = Object.entries(summaryData.byProject)
                .sort((a, b) => b[1].totalMinutes - a[1].totalMinutes);
